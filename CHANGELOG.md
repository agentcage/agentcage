...
- **Web interface round 3: one page per panel, traffic status filters, and a DNS audit trail that actually shows allowed lookups.** (1) The cage view is split into tabbed pages — summary, traffic, dns, secrets, allowlist, capture, logs — each fetching only its own panel over hash routes, replacing the single stacked page. (2) The traffic and DNS pages gained status filter chips (all / allowed / blocked / flagged) and a host filter; both are applied **server-side** (`?decision=`, `?host=` — the same options `cage audit` takes), so the table, the live SSE stream (which now carries the active filters in its URL), and the CLI twin all show the same slice. `cage_dns` forwards the filters and the `/dns` route parses them. (3) **DNS data fix**: the egress supervisor invoked `dns-audit.sh` without `--log-allowed` on all three dnsmasq paths, so only blocked (sinkholed) lookups were audited — the DNS panel was structurally empty on a well-behaved cage that only queried allowlisted names. All three invocations now pass `--log-allowed`, matching the mitmproxy side's `log_allowed` default of true, so every DNS decision (allowed, forwarded, cached, and sinkholed) lands in the audit stream. Existing cages need a `cage update` to pick up the rebuilt egress image.
