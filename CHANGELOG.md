...
- The Lima provisioning script no longer assumes the guest user is named `lima`. Lima 2.x names the guest user after the host user and exports it as `$LIMA_CIDATA_USER`; the script now reads that. Previously `loginctl enable-linger lima` (run under `set -euo pipefail`) would abort provisioning against a non-existent account.
