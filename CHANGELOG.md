...
### Fixed
- The `openclaw` scaffold's nested Podman works again against current `ghcr.io/openclaw/openclaw:latest` images. Recent openclaw images ship a `node` entry in `/etc/subuid` and `/etc/subgid`; the scaffold Containerfile appended its own, creating a duplicate overlapping subuid range that `newuidmap` rejects — so rootless `podman run` inside the cage failed. The Containerfile now adds the entry only when one is missing.
