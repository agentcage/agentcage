...
### Added
- IMAP relay: `policy.require_authentication` (default **true**) rewrites every SEARCH/UID SEARCH command to append three `HEADER "Authentication-Results"` criteria — `dkim=pass`, `spf=pass`, `dmarc=pass`. The upstream IMAP server applies the filter; the response forwards verbatim. The cage thus only sees UIDs of mail the receiving MTA stamped as authenticated. Sequence-numbered `FETCH`/`STORE` are rejected at the command layer so the cage cannot bypass the SEARCH filter by referencing messages by position. Multi-line SEARCH commands containing IMAP literal-string criteria are rejected with `NO`. New audit entries: `kind: imap_search_rewritten`, plus `kind: imap_command, decision: blocked` for the FETCH/STORE rejections. See `docs/configuration.md` ("Inbound message filtering") for the full design.
- IMAP relay: `policy.from_allowlist` (default `[]`) extends the same SEARCH-rewrite mechanism to a sender allowlist. When set, the relay appends `OR FROM "addr1" FROM "addr2" ...` criteria so the cage only sees mail from the listed senders. Composes with `require_authentication`: when both are active, every SEARCH applies both filters. Sequence-numbered FETCH/STORE rejection extends to from_allowlist alone (sufficient to gate the bypass). **Caveat**: IMAP `FROM` is a substring match on the entire raw From header (including display name), so `from_allowlist` is only safe when paired with `require_authentication` — DKIM/DMARC alignment then rejects forged From-domains at the upstream MTA before the SEARCH runs. Documented in detail under "Inbound message filtering".
