...
### Changed (BREAKING)
- The proxy container now installs a default-deny `filter:FORWARD` policy on every cage. Cage→external traffic is dropped unless the destination port is listed in the new `ports.allow` (or `ports.passthrough`, which auto-merges into the effective allow set). Pre-change, only ports `80` and `443` were REDIRECTed into mitmdump's transparent listener — every other TCP and UDP port silently L3-forwarded uninspected. The cage's `domains.allow` gated *which hosts* could be reached but said nothing about *which ports* could exit. An agent that resolved an allowed hostname could exfiltrate over any port (NTP, custom binary protocols, QUIC/HTTP3) with the audit pipeline blind. The new default-deny posture closes that gap.
...
### Added
- `ports.allow` config field lists the TCP+UDP destination ports the cage may reach. Mirrors the shape of `domains.allow`. Default `[80, 443]` preserves the historical HTTP/HTTPS coverage; extend to permit non-standard services (`8448` for a Matrix homeserver, `123` for NTP, `5432` for Postgres, etc.). Inspected ports (= `allow - passthrough`) become `iptables nat:PREROUTING REDIRECT` rules that divert TCP traffic into mitmdump's transparent listener; everything in `allow` but not in `passthrough` runs through `audit.jsonl`, the inspector chain, and the secret injector. Reserved ports for the inspected set (`8443` mitmdump transparent, `8080` mitmdump HTTP-proxy, any `protocol_relays[*].listen` port, any `container.ports[*].container_port` inbound forward) are rejected by `validate_config` because each would conflict with an existing in-process listener — moving them to `passthrough` is the supported escape hatch. Per-entry validation: integers only (YAML strings, booleans, floats rejected), 1-65535 range, no duplicates. Setting `ports.allow: []` disables transparent capture entirely (operator opt-out for L7-only setups); a config validation warning surfaces this as a deliberate posture change. See `docs/proxy-audit-ports.md` for the worked example and trade-off discussion.
- `ports.passthrough` config field lists the subset of allowed ports that bypass mitmdump inspection. Mirrors `domains.passthrough` semantics: a passthrough port not explicitly in `ports.allow` is auto-merged into the effective allow set at quadlet generation, with a warning surfaced at validation time. Each entry installs one TCP and one UDP `iptables -A FORWARD … -j ACCEPT` rule, so UDP services like NTP and QUIC work without separate configuration. Reserved ports are NOT rejected here — putting `8443` or a `protocol_relays[*].listen` port in `passthrough` is fine because passthrough entries never get a REDIRECT rule and don't conflict with locally-bound listeners. When the effective allow set (`allow ∪ passthrough`) is empty, a warning surfaces that the cage will have zero outbound connectivity. Example: `ports: { allow: [80, 443, 123, 993], passthrough: [123, 993] }` keeps NTP and IMAP working alongside the default HTTP/HTTPS audit.
