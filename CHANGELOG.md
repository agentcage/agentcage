...
### Fixed
- The openclaw scaffold's `entrypoint.sh` now `exec`s the openclaw process at the end instead of running it as a child of a `while true; ...; sleep 2; done` respawn loop. Without this fix, PID 2 inside the cage container was a `/bin/sh` and openclaw was a child further down (typically PID ~49). When `agentcage cage restart` (with `graceful_restart_signal: SIGUSR1`, the new openclaw scaffold default) delivered SIGUSR1 via `podman kill --signal=SIGUSR1`, tini correctly forwarded it to PID 2 — but `/bin/sh` has no SIGUSR1 handler, so the kernel's default action (terminate) applied and the entire container exited with status 138 (= 128 + SIGUSR1). systemd auto-restarted, but the in-memory state (Matrix olm sessions, etc.) was lost — the very thing `graceful_restart_signal` was supposed to preserve. With `exec`, the shell is replaced and openclaw becomes PID 2 directly under tini; signals reach openclaw and trigger its in-process restart cleanly. The respawn loop dates from pre-2026.5 openclaw, where SIGUSR1 caused a process exit + entrypoint-loop respawn; 2026.5+ never exits on SIGUSR1, so the loop's only role was crash recovery — already covered by the cage unit's `Restart=on-failure`.
