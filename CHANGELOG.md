...
- **`install.sh` downloads a verified binary instead of installing Python.** It resolves the latest release (through the `/releases/latest` redirect, not the rate-limited JSON API), downloads the tarball for the detected platform, and **refuses to install unless the published sha256 matches** — a truncated or tampered download that still untars no longer becomes an installed binary. It no longer installs Python or `uv`, because nothing on the host needs them. `AGENTCAGE_VERSION`, `AGENTCAGE_BIN_DIR` and `AGENTCAGE_DOWNLOAD_BASE` cover pinning, relocating and mirroring.
...
- **The apple-container egress never started, taking the whole cage's network with it.** `supervisor-egress.sh` hard-fails (exit 16) unless `/proc/sys/net/ipv4/ip_forward` reads `1`, and neither route that supplies it was available on this backend: the Quadlet `Sysctl=` directive is podman/systemd-only and this backend generates no Quadlets, and Apple's runtime mounts `/proc/sys` read-only, so the supervisor's own `sysctl -w` fails even as uid 0. `container run` has no `--sysctl`, so the value now arrives on the kernel command line (`--kernel-arg sysctl.net.ipv4.ip_forward=1`, honoured by Linux >= 5.8). Because the egress IP is both the cage's default gateway and its dnsmasq upstream, the failure presented as a total loss of connectivity inside the cage rather than as a proxy error. The change is scoped to the egress microVM alone — the cage VM gets no kernel arg, the FORWARD policy is still `DROP` with only enumerated ports accepted, and both failure modes (a `container` CLI without `--kernel-arg`, a pre-5.8 guest kernel) still fail loudly rather than leaving a half-configured egress passing traffic.

- **`cage start` on a never-built cage printed a traceback about an internal function.** The error asked whether `build_artifacts()` had been called — something an operator cannot do — and led with the derived wrapper tag (`agentcage-apple-<cage>`), a string that appears nowhere in `cage.yaml`, so grepping the config for it found nothing. It now names the cage and the remedy (`agentcage cage update <name>`), demoting the image tag to trailing detail. Operator-facing failures raise a dedicated `OperatorError`, which the CLI renders as a single `error: …` line; `AGENTCAGE_TRACEBACK=1` restores the traceback for debugging agentcage itself. The dedicated type is load-bearing rather than cosmetic: `click`'s own `Exit` and `Abort` both subclass `RuntimeError`, so a blanket catch turned `--help` into its own text followed by `error: 0`, and `NotImplementedError` subclasses it too, which would have reduced an unimplemented secret backend to a bare `error:` with an empty message.

- **`cage backup` did not carry the build context, so a restored cage that builds from a staged Containerfile could not be rebuilt.** The tarball copied three fixed filenames out of the state dir; it now copies the whole directory minus an explicit exclusion list, records `build_context_included` in the manifest, and reinstalls the extra files on restore. Three further defects were found alongside it. `restore --force` destroyed the existing cage *before* validating the tarball, so restoring a pre-fix backup wiped the cage and then aborted with nothing left standing — extraction and the preflight now run first, which also stops an abort from leaving orphaned podman secrets behind. The keychain name index (`secret_keys.json`) round-tripped through the backup, so on a clean host the fail-closed missing-secrets check in `cage update` passed vacuously and the cage started with no secrets at all; it and `fingerprint.json` are now excluded, and restore refuses to reinstall them even from a hand-crafted tarball. And symlinks were dereferenced, which both copied the target of a link pointing outside the cage bodily into the tarball and turned a dangling link into a backup error: relative in-tree links are now preserved verbatim, while a link that escapes the config dir is dropped with a warning naming it, because carrying it makes the tarball unextractable under tar's `data` filter and dereferencing it exfiltrates host content.

### Changed

- **openclaw's restart signal moved from SIGUSR1 to SIGUSR2.** Upstream commit `fe318b3`, first shipped in the `2026.9.6` image, reserves SIGUSR1 for Node's V8 inspector — so sending it to a caged openclaw gateway attaches the debugger instead of restarting it, silently and with no error. If you restart a caged openclaw by hand, use `pkill -USR2`. `commands.restart` is unaffected and still defaults to true, so no scaffold or config change is required; only agentcage's own regression canary needed updating.
...
- **`agents:` — the in-egress LLM agents get their own top-level namespace.** The two LLM agents agentcage runs inside the egress on your behalf were configured in two different places at two different depths: the decider (which adjudicates the cage's runtime domain requests) hid four levels deep as `domains.auto.decider.provider` — inside the block `domain add`/`domain rm` edit — and the traffic watcher sat at top level as `watcher:` with its LLM fields nested one more level under `agent:`. Both now live under one roster, `agents.decider:` and `agents.watcher:`, with the LLM client fields (`provider`, `model`, `api_key`, `timeout_seconds`, `max_tokens`, `base_url`) **flat on each block** — one grammar, one credential shape across the roster. A config now answers "what is agentic (and what does it cost)" at a glance, and `domains:` is purely the static, operator-authored policy again. The `decider.kind` discriminator is gone (v1 only ever implemented `kind: agent`; the block *is* the agent now — the webhook decider returns as its own shape when it ships). Semantics are unchanged: enable switches, the `agentcage.local` control host, rate limits, per-agent `context` channels and caps, fixed grant defaults, and every CLI command (`watcher status/findings`, `domain add/rm`, `cage grants`) keep their names.
  ...
  **BREAKING migration:** `domains.auto:` and top-level `watcher:` are rejected even when empty, null, or disabled; `kind` (including `kind: agent`) and nested `decider:` / `agent:` wrappers are unsupported. There is no automatic migration or deprecation window. **Manually** move the old blocks to `agents.decider` / `agents.watcher`, flatten the LLM fields, and remove the old keys. Edit the stored `cage.yaml` directly and run `agentcage cage update <name>`, or replace it using `agentcage cage update <name> -c <converted.yaml>` with a complete, manually converted file; this replacement does not transform the old config. Update source copies too. **Rebuild/update the egress along with the config:** generated `proxy-config.yaml` contains canonical agent keys only, with no legacy shadows. Old egress images cannot read them; pushing the config live first can stop watcher monitoring. Stop affected cages before upgrading, then convert and rebuild before resuming workloads — this is not a live schema upgrade. See [the upgrade procedure](docs/how-to/upgrade-agentcage.md#upgrading-to-040-the-agents-namespace).

### Fixed

- Agent `enable` switches now reject non-booleans consistently, including quoted `"false"`. Explicit invalid completion budgets and timeouts are no longer silently replaced with defaults.
...
- **`capture.max_file_size` — the HAR capture file no longer grows without bound.** Capture was append-only with no cap and no rotation, and the docs said so outright ("the capture file grows indefinitely"). Measured on a real cage, a body-heavy workload wrote **222 MB in 20 minutes** of `apt` traffic, most of it base64-encoded package binaries — enough to fill a volume, and enough that the traffic watcher's byte-offset tail could never catch up. The writer now rolls over at `max_file_size` (default 128 MB), keeping one previous generation (`capture.jsonl.1`), so on-disk use is bounded at twice the cap; `0` restores the old unbounded behavior. `agentcage cage har` reads the rotated generation before the live one, so an export taken after a rollover is not silently shortened. Two watcher-side consequences of the same measurement are addressed with it: the tail now **skips to the live end** once it falls more than 128 MB behind, recording a finding that names the skipped byte count rather than silently analysing ever-staler traffic (the writer can outpace the reader — ~11 MB/min written against 1.6 MB/min read at the default 300s interval), and `agentcage watcher status` reports the current backlog so an operator can see the condition instead of inferring it from file sizes. The watcher's single-line cap is also **derived from `capture.max_body_size`** rather than hard-coded at 32 MB: an entry carries four body slots and base64 inflates binary by a third, so the fixed cap sat below the worst case the default body cap permits and a large transfer's entry was discarded as "oversized" even though capture had written it legitimately.

- **The watcher's digest pipeline is hardened against an adversarial cage.** The caged agent controls every byte of its own traffic and can read this code, so every deterministic boundary in the pipeline was a place it could shape traffic to sit just past — and several of these evasions were trivial: pad the first 512 characters of a request body and put the payload after the excerpt; base64 the payload so it is never excerpted; send ten decoy bodies once each so the real one loses a recency tie-break; flood the window with distinct shapes so the budget trims the evidence; exfiltrate right after a scan and push chaff through the audit ring. Each scan now draws one seed from OS entropy (nothing the cage influences) and derives every random choice from it, and the seed is recorded in the scan state so a finding stays reproducible. **Randomised:** body excerpts are head + a random middle window + tail (the tail always covers the end, the middle lands where the attacker cannot predict); rarity tie-breaks for which distinct bodies survive a collapsed group; budget sampling of allowed flows (a random draw, not a computable even spread); cadence, jittered ±50% at unchanged mean cost so the predictable safe window after a scan is gone; and one scan in twenty runs at 4x the digest budget so no window is safely known to be in cheap mode (bounded — this must not reopen the uncapped-spend hole; the create-time spend warning accounts for it). **Evasion as a signal:** the digest carries an `evasion_indicators` block of deterministic counts — distinct shapes before/after collapsing, the largest body-diversity on one shape, binary request bodies to runtime-granted hosts, bodies longer than the excerpt, and audit-ring saturation — computed BEFORE the budget trims, so a flood is on the record even when most of it is cut. Two are strong enough to become findings without the model: a binary request body to a runtime-granted host (exactly how one hides an exfiltration from an excerpt-based review), and a saturated ring (evidence likely evicted unreviewed). Low-and-slow exfiltration across windows is explicitly out of scope; it needs memory across scans. **Also from the same live testing:** a fast model answered the forced `review` tool call with EMPTY arguments and put its whole analysis in the message text as prose, three scans running, each correctly failed closed as "no usable review tool call" — so every window went unanalysed while the provider was in fact responding. The watcher now retries ONCE when a reply is schema-non-compliant (no tool call, empty args, or `findings` not a list), restating the contract; network and provider errors are deliberately not retried, since the next tick already retries the window and doubling calls on a dead provider only doubles the bill.

- **Sane cost defaults: the watcher can no longer run up a surprise bill.** There was no spend guardrail of any kind. `max_flows` bounds how many samples are sent, not how large they are, so nothing bounded cost — and the validator accepted `interval_seconds: 60` with `max_flows: 2000` in silence, which is roughly **1.2 billion input tokens a day**, a five-figure monthly bill. A new `watcher.max_digest_tokens` (default 8000, `0` disables) is a hard ceiling on the digest, applied last so it bounds the whole prompt; when it bites, allowed flows are dropped oldest-first while blocked and flagged ones are held back from the cut, because those are what the scan exists to see. The cadence default moves from 300s to **900s** — chosen for the bill rather than for latency, since this is an after-the-fact auditor by design. Together the defaults hold a cage to ~885k input tokens/day: about **$47/month on a frontier-priced model and $2.50 on a fast one** at measured September 2026 prices, where the old defaults reached ~$1,000/month on the same frontier model. `agentcage watcher status` now reports the actual digest size against the budget, so an operator sees real usage rather than only the ceiling, and any configuration that could exceed 5M tokens/day warns at `cage create` / `cage update`.

- **The watcher's digest collapses repeated flow shapes, cutting the model bill ~5x.** Real cage traffic is dominated by repetition — a poller hitting one endpoint, a package manager walking a mirror — and sending the model forty near-identical samples bought nothing but tokens. Samples now collapse on their SHAPE (host, method, path with per-request ids and hashes templated out, decision, status) into one sample carrying `repeated`, `first_ts` and `last_ts`. Measured on a real cage: **61 samples became 17, 18.4% of the prompt payload**; projected at a busy cage filling `max_flows: 200`, that is 84k tokens per scan down to 16k. Repetition also becomes *explicit* rather than something the model must notice across samples — a beacon reads more clearly as "43 identical requests between two timestamps" than as 43 separate entries, and the digest note now tells the model how to read a collapsed sample. **Evidence is deliberately not the thing being saved on:** up to three DISTINCT request-body excerpts survive per group, because the revocation this feature demonstrated fired on a request body carried by an *allowed* flow, and a first-wins exemplar would have shown the model only a benign body from a group whose later member was the malicious one. Which distinct bodies are kept is itself security-critical: keeping the FIRST few is the worst choice, because an attacker buries one malicious body in a run of benign ones and a late arrival is then discarded — caught on a real cage, where six benign graphql bodies followed by an exfiltration body silently lost the evidence with a first-three cap. Retention is ranked by RARITY instead, which is the threat's own shape (polling repeats, exfiltration does not): the modal body is kept as a baseline for comparison, then the rarest, and `distinct_request_bodies` stays honest when the list is capped. Cheaper reductions were measured and rejected for exactly this reason: dropping bodies from allowed flows reaches 6.6% of baseline but retains zero allowed-flow bodies, which would have left the exfiltrating grant in place. Opt out with `watcher.dedup_samples: false` (a real boolean; a quoted `"false"` is rejected, the same trap `auto_revoke` guards).

## [0.36.0] - 2026-09-04

### Fixed

- **The traffic watcher no longer sends injected secret values to its model, and no longer drops evidence on a large capture backlog.** Eight defects from a follow-up review of #340. (1) **Secret leak (highest severity):** `_sample_capture` excerpted the capture entry's top-level `path` into the digest, but that field is snapshotted in `addon.request()` *after* `injector.inject_request` runs — and an `inject_body: true` rule rewrites the placeholder inside `flow.request.url`, the documented `?key=` query-string case — so a real API key could ride to a third-party model, breaking the feature's own "secret NAMES may appear, values never" invariant. The path now comes from the INBOUND (pre-injection, placeholder-safe) snapshot, falling back to a query-stripped path when no inbound url was recorded. (2) The oversized-line guard measured the whole accumulated read instead of the un-terminated line, so any backlog larger than `_MAX_LINE_BYTES` ended its read mid-line, looked "oversized" at every chunk boundary, and silently discarded one COMPLETE entry per tick with a false warning — 25% of a 60-entry file in the regression test. The chase loop now only extends when the read produced no complete line at all, and the drop guard measures the line. (3) The reset cursor was cleared inside `_read_capture`, which only *stages* an offset, so the last catch-up tick of a multi-tick reset dropped the `window_seconds` filter even when its scan then failed and the retry re-read the same bytes unfiltered; `_commit_capture` retires it alongside the offset now. (4) `auto_revoke: false` discarded the model's `allowlist_removals` with no record at all, though the field's own documentation says they "degrade to findings" — the posture operators are told to start with threw away each scan's most actionable output; they are now recorded as "revocation recommended" findings. (5) `cage edit` classified `watcher` as live-only and never refreshed the units, but the grants volume its findings are written to and the `Secret=` directive for its api_key are both decided at unit-generation time — so a freshly enabled watcher on a cage with `domains.auto` off wrote findings into the container's ephemeral layer and `watcher findings` reported the silent all-clear the feature exists to prevent. (6) **Blocklist mode is now rejected:** there `DomainInspector._baseline` IS the block list, so the digest presented blocked domains as the allowlist and a baseline recommendation would have told the operator to run `domain rm` — removing a block and WIDENING egress from a narrowing-only auditor. Guarded at config time and re-checked in-egress against a hot reload. (7) `max_tokens` was in the anthropic request body only, so the shared bound was silently ignored on openai and openrouter. (8) `services.expected_secrets` never learned about either egress agent's api_key, so `agentcage secret set <cage> WATCHER_LLM_KEY` — the command the docs prescribe — called the key an "orphan" and `check_secrets` gave no preflight warning when an agent-enabled cage was deployed without it.

- **OpenClaw scaffold works with OpenClaw 2.0 (v2026.8.1+, tested against v2026.9.1).** Four changes, found by running the phase-8 canary against the new image. (1) *Writable `~/.cache` tmpfs*: 2.0 moves sessions/transcripts into SQLite and its read-only worker needs a fallback temp dir under `/home/node/.cache`; on the scaffold's read-only rootfs the gateway exited at boot with "Unable to create fallback OpenClaw temp dir: /home/node/.cache/openclaw-0". The scaffold cage.yaml now mounts `/home/node/.cache` as a 1G tmpfs. (2) *Forwarded headers are stripped, not synthesized, on inbound reverse flows*: the egress addon set `X-Forwarded-For` to the client address it observed, but rootless pasta port publishing rewrites the source so that value is always the egress's **own** network address — an attribution chain that never resolves to a real client. openclaw 2.0's ingress attribution rejects exactly that shape on Gateway-authenticated routes with HTTP 403 `proxy_attribution_required` (trusted-proxy traffic must resolve to a non-loopback client; `allowRealIpFallback` only consults an `X-Real-IP` *header*, which nothing here sets), so every host→Control-UI request through the published port 403'd. The addon now deletes `X-Forwarded-For` / `X-Forwarded-Proto` / `X-Forwarded-Host` / `X-Real-IP` / `Forwarded` on inbound flows (client-forged values included — previously they were merely overwritten), and the gateway attributes the connection as a direct remote client, which is what the relay effectively is. The `Host`/`Origin` rewrite is unchanged, and the observed source still lands in the audit log — it was read from the connection, never from the header. (3) *`gateway.trustedProxies` dropped from the generated openclaw.json*: with headerless inbound flows the declaration is not just useless but fatal under 2.0's semantics (a trusted source without a resolvable forwarded chain is unattributable); the vestigial `CAGE_PROXY_IP` env plumbing that fed it is gone too. The setting dated from the pre-`ce05c6c` 3-service shape whose dedicated reverse proxy really did rebuild client headers. (4) *In-cage `openclaw` CLI invocations need `--as-root`*: the 2.0 CLI reads `openclaw.json` and the SQLite session store directly instead of asking the gateway over its socket; in this cage those files belong to the gateway's root user, so the default uid-1000 `cage exec` session fails with EACCES ("Config file is not readable by the current process"). The scaffold help text, README, and the phase-8 health check now pass `--as-root` for `openclaw ...` commands. E2E: new 8.6b asserts `trustedProxies` stays absent; the 8.0 setup patch now pins `secrets.backend: plaintext` so the canary also runs on dev hosts where systemd-creds would win the `auto` election (cage-level `podman_secrets` have no creds-decrypt materialization step there — only `secret_injection` entries do — so `cage create -s` failed with "no such secret" before the first assertion; pre-existing gap, tracked separately).

### Added

- **The traffic watcher — an opt-in, in-egress LLM agent that audits cage traffic after the fact.** The domains decider guards the front door (no new domain is granted until an LLM cybersecurity expert adjudicates the request), but nothing re-examined what the cage *did* with its egress — the shape of traffic over time is exactly where exfiltration, C2 beacons, credential relays and inbound prompt-injection payloads hide from every per-request check. A new `watcher:` block in cage.yaml enables a second LLM agent that runs **inside the egress container** (like the decider — not a host daemon: agentcage deleted its last host-side watcher in the egress-local DNS-apply rework, and the egress supervisor already provides the process supervision) as an asyncio scan loop sibling of the grants sweeper: every `interval_seconds` it builds a digest of the window's traffic — audit decisions (EVERY audit entry funnels through one bounded in-memory ring — ordinary HTTP decisions via the addon's `_log` included, and ALLOWED traffic is ingested even when `logging.allowed_requests` suppresses it from the durable log, because exfiltration patterns live in traffic that was allowed), per-host aggregates, inspector triggers, secret *names*, the decider's own grant/deny record, and capped samples of `capture.jsonl` (the HAR source, tailed incrementally by a byte offset that advances only past complete lines and tracks file identity, so torn writes and rotation lose nothing) — and asks the model, prompted as a senior traffic auditor with the same untrusted-data/prompt-injection hardening as the decider, to flag anything suspicious. The scan's LLM call runs via `asyncio.to_thread` (mirroring the decider), so a slow provider can never stall mitmproxy's event loop; the ring is drained in ingestion order (no timestamp cursor for a future-dated entry to poison), a failed scan pushes its drained batch back and does not commit the capture offset (no evidence is lost to an LLM hiccup), and the watcher's own records are never fed back into the model's evidence. **Trust model — it can only ever narrow**: the watcher may revoke the *runtime grants* its analysis damns (through the exact `dom.revoke` → overlay-persist → DNS-republish chain the `POST /v1/allowlist/removals` endpoint uses, gated on `auto_revoke`), and can never grant, never edit the operator's static baseline ("baseline immutability from the egress"), never touch `never_grant` policy — baseline edits are emitted as *recommendations* the operator applies with `agentcage domain rm`. Fail-closed on every LLM outcome: an error, timeout, missing tool call, wrong tool name, or a verdict violating the output contract's shape is a *recorded* `watcher_scan_failed` finding (throttled so a dead provider cannot flood the findings file), never a silent all-clear and never a revocation spree. Secret hygiene: only the *inbound* (placeholder-safe) view of a body is ever excerpted — the outbound view holds the real secrets secret-injection put on the wire and contributes metadata only; sensitive headers are dropped by name. Reuses the decider's machinery directly: the LLM wire client was factored into shared module-level helpers (`llm_tool_call` / `parse_tool_args`) that both agents import in-egress, and the watcher's `api_key` follows the decider's exact credential chain (an egress-only `env:`/`systemd-creds:` secret, stripped from the cage env, staged into the egress's tmpfs, auto-resolved DNS for the provider host). Findings are written to the grants volume (`watcher/findings.jsonl`, host-visible on every backend) and re-emitted into the audit stream as `watcher_finding` entries with an inspector-shaped record so `cage audit --inspector watcher` / `--decision flagged` surface them next to the inspector findings that caused them (their severity rides the model's info/low/medium/high/critical vocabulary, which the audit tooling's ladder now ranks alongside the inspector one, so `cage audit --severity warning` sees a `high` finding); revocations audit as `watcher_revoke` with per-revocation persistence (the removal endpoint's posture) and an expiry-aware `still_allowed_by_baseline` flag when an active static entry also covers the domain — never a bare "blocked" the traffic would disprove. `secret list` classifies the watcher's key as `watcher` (not `orphan`), the same footgun fix the decider's key got. New read-only host CLI `agentcage watcher findings <name>` / `watcher status <name>` (VM cages pull guest-side over `limactl shell` with `pull_grants`' sentinel protocol). Hot-reload wired like domains.auto — enabling, disabling, or re-pointing the model takes effect on a live config edit, and a reload that leaves the block unchanged is a no-op that keeps the scan state (a malformed rebuild keeps the last working watcher running). Works identically on all three backends (the audit funnel and grants volume already are). Disabled by default: a cage without a `watcher:` block imports nothing, starts no task, resolves no extra DNS, and stages no credential. See docs/how-to/run-the-traffic-watcher.md for the operator workflow (including what a caged agent sees when a grant is revoked) and docs/explain/traffic-watcher.md for the design.
...
- **`POST /v1/allowlist/removals` — the caged agent can give back a runtime grant it no longer needs.** The Policy API could widen (request → decider → grant) but the only narrowing paths were operator-side (`agentcage domain rm`, TTL expiry), so a cage that requested a domain for a one-off task kept that egress forever unless the operator noticed. The new control-host endpoint is the request endpoint's mirror image in trust terms: a removal only ever **shrinks** the agent's own egress, so no decider is involved and no justification is required (an optional `reason` lands in the `policy_removal` audit line). Scope is deliberately **live runtime grants only** — a grant the host reconcile already promoted into the static baseline is indistinguishable from a domain the operator added by hand, and the egress never edits the baseline (the same immutability invariant that routes promotion through the host-side `domain add` machinery), so those return `403` naming `agentcage domain rm`; a domain that is not a grant at all returns `404`, and a removed grant whose domain still suffix-matches the baseline says so via `still_allowed_by_baseline: true` rather than letting the agent believe it narrowed more than it did. Removal takes effect the way a grant does, in reverse: the domain leaves the in-memory L7 allow set immediately, the shrunk zone list is republished so the supervisor drops it from dnsmasq within ~1s, and the overlay entry is deleted so the removal survives an egress restart. No host-side changes were needed: the reconcile's merge-on-write already computes `on-disk − this-tick's-removals`, so a cage-side removal landing mid-tick simply stays absent; the residual race (a concurrent reconcile promotes the grant first) resolves to "operator-owned, 403 on retry" — narrowing converges to the operator's explicit state, never silently widens. Removal is not a ban: the agent can re-request the domain later and the decider adjudicates fresh. Shares the request endpoint's per-cage rate-limit bucket, rides the same `auto.enable` master switch, and is reported in `/v1/health`'s features as `removal`.

### Fixed

- **`POST /v1/allowlist/removals` now matches the baseline only, is expiry-aware, guards the mode, and rate-limits before validating.** Four review follow-ups on #337. (1) Both the `still_allowed_by_baseline` flag and the 403-vs-404 branch called `DomainInspector._matches`, which suffix-matches `domain_set` = baseline ∪ *live grants* — not the baseline alone — so with sibling grants for both `x.com` and `sub.x.com`, removing `sub.x.com` lit the flag off the surviving `x.com` *grant* (the flag lied about which source keeps the domain reachable), and removing a non-grant domain covered only by a sibling grant's suffix returned 403 "operator baseline" instead of 404. A new `DomainInspector.matches_baseline` walks `_baseline` alone (mirroring the baseline half of `is_grant_only`; `_matches` itself is untouched — other callers rely on the union) and is now used for both the flag and the 403-vs-404 branch. (2) The flag ignored baseline expiry: a baseline entry can be expired (`domains.expires` / `expires_at`), in which case L7 blocks the domain, so the flag mislead in exactly the re-grant-over-expired-baseline scenario its comment cited. The flag now lights only when an *active* (non-expired) baseline suffix covers the domain; `_matched_expired` cannot be leaned on here (it consults `domain_set`, so a still-live sibling grant would flip the flag true), so the matching baseline suffixes are walked directly with the same fail-open-on-malformed-expiry posture. (3) `_handle_removal` had no allowlist-mode guard, so a hot-reload to blocklist mode inverted the 403 reason / flag; it now 400s up front like the request endpoint. (4) Invalid-domain removals were audited *before* the rate limit consumed a token, so the caged agent could emit unbounded `policy_removal` audit lines; the rate-limit gate now runs first, keeping the existing 429 body/audit and invalid-domain audit shapes (the latter now reachable only within the rate limit).
...
- **`domains.auto` — an opt-in, auto-managed domain allowlist with a built-in LLM cybersecurity-expert decider.** A caged agent often needs exactly one more domain than the operator listed, and the escape hatches were either to widen the static allowlist or to have the agent stop and ask. Both new capabilities are served by the egress proxy on a reserved control hostname, so they keep working under full default-deny: the agent can `GET` the effective domain policy (static baseline plus runtime grants), and `POST` a request for a new domain. The egress never self-authorizes — every request is adjudicated by the **decider**, an LLM agent prompted as an adversarial cybersecurity expert that scrutinizes the request's justification as a claim to be attacked, not accepted. The decider's provider is configurable (`decider.kind: agent` with an OpenAI-compatible `base_url` — OpenRouter, Anthropic, a local vLLM, …) and its API key is staged as a podman secret (from the host keychain on macOS); the decider host itself and the relay upstreams are added to the DNS allowlist automatically so the adjudicator can actually be reached. Operators can tell the decider what the cage is *for* via operator-provided cage context, which is passed to the prompt in a delimited block the request cannot forge. Grants are time-limited (TTL), audited end-to-end (`policy_grant` / `policy_grant_removed` events in `cage audit`), persisted across egress restarts via a grants overlay, and applied as **live DNS inside the egress** (#333) — there is no host-side watcher process, and the overlay is republished on every reconcile with expired entries skipped at publish. Hardening merged before first release: IP-encoding hostnames (nip.io / sslip.io / xip.io / traefik.me and every future clone — the encoding itself is matched, not a service denylist) are refused structurally before the decider ever sees them (#334); the resolved peer address of a granted domain is validated so a name that resolves to an address it must never reach never enters the overlay (#335); and the control plane carries SSRF guards, runtime caps and type hardening from the round 7–11 review funnels. The operator can revoke a grant with `agentcage domain rm` or promote it into the static `cage.yaml` baseline. Works on all three backends — `container`, `vm`, and `apple-container` (parity: decider-host DNS, systemd-creds staging, and `policy_api.py` shipped in the egress image). Disabled by default: a cage without `domains.auto` behaves exactly as before. Closes #330.
- **`agentcage domain add --expires-in` — time-limited static grants.** `domain add` gained `--expires-in`, and `cage.yaml` domain entries gained `expires`, so a static allowlist entry can be temporary — expiring on its own instead of waiting for someone to remember to remove it. The static baseline and the runtime grants overlay now share one TTL model, so a promoted grant keeps the expiry it was granted with.

## [0.33.0] - 2026-08-29

### Fixed

- **tmpfs-mask copy-up is now an explicit, per-entry decision with the same meaning on every backend.** Nothing in agentcage said whether a mask should come up empty or holding a copy of what it covers, so each runtime's default decided — and they disagree. Measured on real hardware against a project with a real `pre-commit` hook and a `.claude/settings.json`: on `container`/`vm` the `/workspace/.git/hooks/` mask held all 15 of the host's hooks including the real `pre-commit`, and `/workspace/.claude/settings.json` read back the host's file, all owned `0:0`; on `apple-container` both masks were genuinely empty. Podman appends `tmpcopyup` to every tmpfs whose spec names neither copy-up option (`pkg/util/mountOpts.go`), while Apple's `container run --tmpfs` takes a bare path and has no option channel at all. Neither outcome was chosen, and the podman one is the worse of the two: the masks are `noexec`, so a copied-up hook can never execute in the cage anyway, and because the runtime performs the copy as the user-namespace root without replaying the source's ownership, the content landed root-owned — with #321's `mode=1777` sticky root the workload could create a new hook but overwriting the host-owned `pre-commit` returned `Permission denied` and deleting it `Operation not permitted`. Containment was never affected (verified: after `cage stop` the host still had its 15 hooks, its `pre-commit` unmodified, and the cage-created hook absent — #170/#173 stay closed); this was a semantics and usability bug. `container.tmpfs` entries now take `tmpcopyup`/`notmpcopyup` as a first-class option, and generation pins **`notmpcopyup`** on any mask entry that declares neither — "unspecified" therefore means *empty* on both backends instead of whatever the runtime happens to do, which is both what a mask is normally for and the only default apple-container can honor without a host source. Entries that are not inside another mount (`/tmp`, `/var/cache`, …) are left completely alone: their contents are the image author's intent, exactly as with #321's `mode=` pin. The scaffolds now split the two masks by intent — `/workspace/.git/hooks/` carries `notmpcopyup` (empty, as `scaffolds/claude-code/cage.yaml.j2` has always claimed but did not deliver on podman), and claude-code's `/workspace/.claude/` carries `tmpcopyup` so a caged agent reads the project's `settings.json`, commands and subagents as its own throwaway copy; the scaffold comment and README claim that project-level Claude config "must be baked into the cage image" is retired. On `container`/`vm` the option reaches podman verbatim, so the content is present before the workload starts, and a new `ExecStartPost` on the cage unit hands the copy to the cage user (`container.user`'s numeric uid, else 1000) via `nsenter -t <pid> -U -- chown -Rh` against `/proc/<pid>/root/<target>` — the user namespace this unit's own user owns, so the host's `chown` runs with the capabilities it needs without a `podman exec --privileged` that would have handed a full capability set to the cage *image's* `chown` binary; an `ExecStartPre` cannot do this because it runs before the container exists and there is no tmpfs yet. The hook is `-`-prefixed and always exits 0, so an ownership repair that cannot complete never fails a start; `-Rh` (with GNU chown's `-P` recursion default) means a symlink planted inside the mask cannot redirect it, and only the in-cage tmpfs is touched — never the host source. On `apple-container`, where the option cannot reach the runtime, copy-up is emulated the way `np` binds already are (#311/#316): the host directory the mask covers is mounted **read-only** under `/run/agentcage/masks/`, and a new cage-init stage C'' replays it into the tmpfs and chowns it to uid 1000 before the workload execs — so unlike podman there is no window where the seed is unwritable. Seeding is skipped, without creating anything on the host, when the host directory does not exist (#320's mount-point bookkeeping would otherwise have a stray `.claude/` to account for), when the target is already an `np` tmpfs (#325's double-tmpfs avoidance), and when the source resolves outside the bind it came from — a repository shipping `.claude -> ../../.ssh` is refused with a warning rather than turned into a fresh read-only window onto a host path the operator never shared, which the bind alone does not expose because an in-guest symlink resolves in the guest. `cage create`/`update` warns when a `tmpcopyup` cannot be emulated on apple-container (a mask over a named volume or an `np` bind, or a tmpfs over a plain image directory), and stops listing `tmpcopyup`/`notmpcopyup` among the options that backend drops, since they are now honored. apple-container cages need a `cage update` to rebuild the wrapper image with the new cage-init. Closes #328.
- **The `/workspace/.git/hooks/` and `/workspace/.claude/` tmpfs masks are now writable by the cage workload.** With `user: "1000:1000"` (the scaffold default) the masks came up owned by the user-namespace root at mode `0755`, so the workload could not write a single byte into its own mask — observed on macOS/`vm` with Lima 2.2.0: `mount` showed `tmpfs on /workspace/.git/hooks type tmpfs (rw,...,mode=755,uid=524288,gid=524288)` and `cage exec ... 'echo x > /workspace/.git/hooks/pre-commit'` returned `Permission denied` for uid 1000 while `--as-root` succeeded. Root cause is not platform-specific — the same failure was then reproduced on Linux CI (`ubuntu-24.04`, rootless podman) once the e2e below was repaired, with an identical `mode=755` tmpfs owned by an unmapped uid and the same EACCES for uid 1000: neither OCI runtime gives an option-less tmpfs the kernel's default `1777` root mode — runc copies the mountpoint's mode in `mountEntry.createOpenMountpoint`, crun does the same in `append_tmpfs_mode_if_missing`, and both skip it only when the spec already carries an explicit `mode=`. For a tmpfs masking a path inside a host bind-mount, the mode being copied is the *host* directory's, which has no relationship to the cage's uid space. `quadlets.py` now appends `mode=1777` (`/tmp` semantics — world-writable, sticky) to exactly those `container.tmpfs` entries whose target sits at or below a `container.volumes` bind-mount target, and only when the operator did not declare a `mode=` themselves; entries over image directories (`/tmp`, `/var/cache`, …) keep inheriting the image's mode. The mount-topology lookup both this and #320's mount-point cleanup need is now a single shared `volume_mounts.enclosing_mount()`, and `quadlets.py` drops its private copy of `_mask_mountpoint_dirs()` for the `volume_mounts.mask_mountpoint_dirs()` #325 lifted out of it — the one-line follow-up #325 left open. The mask's containment is unchanged — the tmpfs is still private to the cage mount namespace and mounted `rprivate` (cage writes never reach the host), `noexec,nosuid,nodev` are untouched (nothing planted there is executable), and the sticky bit keeps one in-cage uid from clobbering another's entries. The same fix applies to the `container` and `vm` backends, which share the quadlet generator. `apple-container` (which learned to honor `container.tmpfs` in #325) is **not** covered: Apple's `container run --tmpfs` takes a bare path with no option channel, so the mask lands there with the same inherited mode and the fix has to be a `cage-init.sh` chown analogous to the one #311 added for `np` seeds — tracked separately. Separately, e2e phase 6.11 — the check that was supposed to catch this — created its project directory under `/tmp`, which `quadlets.py` rejects as outside `$HOME`, so `cage create` failed and the phase reported SKIP ("no podman / buildkit available") on every runner instead of ever exercising the mask; the directory now lives under `$HOME` and is chmod'd to a real checkout's `0755` (`mktemp -d` makes `0700`, and under `userns: keep-id` the host user keeps its own uid in the cage, which is not the workload's uid 1000 — so a `0700` `/workspace` denied even search permission and every path below it failed before the mask was reached), the swallowed `create` output is surfaced in the skip reason, and the in-cage write's stderr plus a snapshot of `id`, the three path components' modes and the tmpfs line from `/proc/self/mounts` are reported on failure. 6.11 now runs and passes for real instead of reporting SKIP. Closes #321.
- **A `tmpfs:` mask under the workspace bind-mount no longer litters a non-git project with a stray host `.git/` and `.claude/`.** The #170/#173 scaffold masks target `/workspace/.git/hooks/` and `/workspace/.claude/`, which sit under the `${PROJECT_DIR}:/workspace:rw` bind. A bind shares inodes with its source, so when the OCI runtime creates those mount points it does so *on the host*: pointing a cage at a directory that is not a git repo left `.git/` (containing only `hooks/`) and `.claude/` behind in the operator's project, and they survived `cage stop`. `git status` still correctly reported "not a git repository" — an empty `.git/hooks/` is not a valid repo — but the ubiquitous `test -d .git` shell idiom misdetected the directory as one, and the project was silently polluted with directories the user never created. The fix is **not** to skip the mask when `.git` is absent: that would reopen the #170 cage->host git-hook pivot for any `.git` created later, including one the cage creates itself, so the masks stay unconditional. Instead the cage quadlet now carries a matching `ExecStartPre`/`ExecStopPost` pair per bind root. The `ExecStartPre` runs immediately before the container starts and records which of the mask mount points — and the parent chain they imply, so cleaning `<project>/.git/hooks` also retires the `<project>/.git` the same mask created — did not yet exist on the host; the `ExecStopPost` removes exactly those, deepest first, with `rmdir`. `rmdir` *is* the "only if empty" guarantee, so a directory that was pre-existing (never recorded) or that anything wrote into (rmdir fails) is always left alone, with a journal warning rather than a failure. The teardown line is prefixed `-` and ends `exit 0`, so a cleanup that cannot complete never fails `cage stop`/`destroy`; it is idempotent, safe to run when the start hook never ran, and re-records from scratch on every restart cycle. Removal is confined to the bind root by construction (the paths are derived from the bind source plus the mask's relative sub-path) and, at runtime, by a `realpath` containment check plus a symlink test, so a symlink swapped in after start cannot redirect a removal outside the project directory. Masks that resolve outside any host bind — `/tmp`, a target under a named volume, or under an `np` bind whose writes land in a `%t` upperdir — are correctly left untracked, since nothing reaches the host there. Host paths reach the unit base64-encoded because a systemd `Exec=` line applies its own quoting before `/bin/bash` does and no single escaping of a path like `~/My Project` survives both layers. Affects the `container` and `vm` (rootless-podman quadlet) backends; `apple-container` never wired `container.tmpfs` at all (#120), so it could not exhibit the bug — and still does not apply the masks, as its existing SECURITY-RELEVANT drop warnings say. Closes #320.
- **The shared `apple-container` egress image is now tagged by content, so an in-release security fix to the egress actually ships.** The image was tagged `localhost/agentcage-egress:<agentcage version>` and `_build_egress_image_if_missing()` skipped the build whenever that exact tag was already present locally. Nothing about the image's actual contents — `Containerfile.egress`, `supervisor-egress.sh`, the mitmproxy addon and the inspectors/relays/transforms trees — took part in the rebuild decision, so any fix landing in the image between releases silently never reached a host that already held the tag. Measured on a real Mac: with a `0.32.0` image built before the #186 proxy-log hardening, `grep -c _ensure_log /opt/agentcage/supervisor` inside the running egress VM returned **0**, `audit.jsonl` was mode **0644** (world-readable — exactly the condition #186 fixed) and `capture.jsonl`/`proxy.log` did not exist; `cage create` just printed "Egress image ... already present; skipping rebuild". After a forced `--no-cache` rebuild the same checks returned **9** and all four log files were **0640**. Same agentcage version, same command, opposite security posture, decided purely by whether a stale tag happened to be there. The tag is now `localhost/agentcage-egress:<version>-<12-hex sha256>`, where the digest covers the sorted (relative path, contents) pairs of the Containerfile plus every file it `COPY`s — the source list is parsed out of the Containerfile's own `COPY` directives rather than hardcoded, so a newly copied file joins the digest automatically. A changed supervisor therefore yields a tag the host cannot already have, the "already present" probe misses, and the image rebuilds with no flag. The digest is content-addressed rather than monotonic, so reverting a change reuses the image already built for it; `__pycache__`/`.pyc` are excluded to keep the tag stable across interpreters, and files in the build context the egress never copies (e.g. `Containerfile.helper`) do not force pointless rebuilds. `--no-cache`/`--pull` keep their existing meaning — they still force a rebuild even when the tag is present. Superseded egress tags are left on disk rather than pruned, because sibling cages may still be pinned to them. The `container` (rootless podman) backend was not affected: it rebuilds the egress unconditionally and relies on podman's layer cache, which invalidates on a changed `COPY`. Closes #312.
- **`apple-container` launchd autostart no longer silently no-ops over SSH.** PR #184 (0.21.15) wrote `~/Library/LaunchAgents/io.agentcage.<cage>.plist` and called `launchctl bootstrap gui/<uid> <plist>`, then reported success. But on a managed Mac driven over SSH, `launchctl list | grep io.agentcage` showed nothing — the cage didn't auto-start until the user logged out/in. Root cause: `~/Library/LaunchAgents/` plists live in the per-user **GUI** launchd domain, which is only addressable from a session that owns the Aqua console. An SSH session runs in the non-GUI `user/<uid>` context: `launchctl print gui/<uid>` fails with "Domain does not support specified action", and `launchctl bootstrap gui/<uid>` exits 0 but silently no-ops because the GUI domain isn't actually reachable from the SSH context — `/dev/console` is owned by `root`, not the service user, and `who -u` is empty. The operator got a misleading "installed" status with no indication the immediate-load had done nothing. The fix makes the immediate-load **best-effort with honest status**: the plist file is always written (it IS the persistence — launchd auto-loads it at the next Aqua login regardless), but `launchctl bootstrap gui/<uid>` is only attempted when `launchctl print gui/<uid>` confirms the GUI domain is reachable from the current context; when it isn't (the SSH/non-GUI case), the bootstrap is skipped and the operator gets a clear informational message — "plist written; autostart will activate at next GUI login (immediate-load not available from this SSH/non-GUI context — see #185)" — instead of the silent no-op. The local Terminal.app case (GUI domain reachable) still gets the immediate bootstrap as before. The issue author's key insight: the immediate-load was convenience, not correctness — the plist on disk was always the persistence mechanism — so the cure (silent failures, misleading status) was worse than the disease. Tests now assert the reachability-aware behavior (probe mocked, bootstrap attempted when reachable, skipped-with-message when not, plist always written) rather than a raw `launchctl list` grep, which is the wrong probe from SSH — it can't see the GUI domain either. Follow-up to #184. Closes #185.
- **The inspector chain now runs in a thread executor instead of blocking the asyncio event loop.** The SMTP relay's `_run_inspectors` and the mitmproxy HTTP addon's `request`, `response` and `websocket_message` hooks all ran the secrets/entropy/content-type/body-size chain synchronously on the loop. For a 5 MB DATA payload through 19 secret patterns plus entropy and content-type checks that is ~50–300 ms of CPU work during which every other client session, the IMAP relay, and any HTTP request flowing through mitmproxy all wait. The chain is now factored into one shared helper — `inspectors/_chain.py:run_inspector_chain` (sync core `run_inspector_chain_sync` + an async wrapper that dispatches via `loop.run_in_executor(None, ...)`) — used by both the SMTP relay and the HTTP addon, so the asyncio loop stays responsive while body inspection runs in a worker thread. The exact return shape, short-circuit-on-block, `ctx.prior_results` accumulation, bypass/flag audit logging and exception behaviour of the previous sync path are all preserved, so callers' semantics are unchanged. Built-in inspectors are pure functions of their `InspectionContext` plus config set once in `configure()` (verified by reading each inspector — no module globals or class-level caches touched on the inspect path), so running them off-loop on the default `ThreadPoolExecutor` is safe. Deferred follow-up from PR #86 (C1). Closes #223.
- **`agentcage doctor` no longer hard-fails on an apple-container-only macOS host, and the apple-container e2e phase now skips cleanly on runners without nested virtualization.** GitHub's hosted `macos-26` runner is itself a VM, so Apple's Virtualization.framework refuses to boot nested VMs with `VZErrorDomain Code=2 "Virtualization is not available on this hardware."`, which blocked wiring `tests/e2e/phase_apple.sh` into CI (issue #215) and made the phase error mid-run instead of reporting a clean skip. `phase_apple.sh` now probes for nested virt (the `ImageOS` env marker set only on GitHub-hosted image runners, plus the `kern.hv.supported` sysctl — the documented Hypervisor.framework availability sysctl — for other VM hosts) and exits 0 with a clear `SKIP: nested virtualization unavailable … (see issue #215)` message when the host can't run nested VMs, while still running the real e2e unchanged when nested virt IS available; an `AGENTCAGE_APPLE_E2E_FORCE=1` escape hatch bypasses the probe for self-hosted runners. Separately, `agentcage doctor` flagged a missing Lima as `error` on macOS unconditionally, so an apple-container-only host (Lima absent but apple-container fully ready and the chosen backend) could never exit 0. `check_lima` is now backend-aware: when the apple-container prerequisites are met, Lima-missing is downgraded to `warn`, so `doctor` exits 0 on an apple-container-only macOS host; the hard `error` is kept only when neither backend is usable. The apple-container prerequisite probe (`container system status`) is now invoked at most once per doctor run and reused for both the Lima backend-awareness decision and the dedicated apple-container check, rather than twice. The CI-status note (the three options — paid `macos-26-large`/`-xlarge` runners, self-hosted Apple Silicon, or keep manual-only) is documented in the phase header; none are in-repo fixes. Closes #215.
- **Workspace bind-mount no longer exposes host `.git/hooks/` to cage writes (cage→host git-hook pivot).** Every first-party scaffold that bind-mounts `${PROJECT_DIR}:/workspace:rw` (claude-code, codex, pi, arch, busybox, debian, ubuntu) handed a caged agent write access to the host project's `.git/hooks/`. A malicious in-cage agent could plant `.git/hooks/pre-commit` (or `post-checkout`, `post-merge`, …) as an executable; the next host-side `git commit`/`git checkout` runs it as the host user — a full cage→host pivot. Each affected scaffold's `tmpfs:` block now overlays `/workspace/.git/hooks/` with an empty, `noexec` tmpfs: the cage sees a transient `.git/hooks/` (writes vanish on stop), and the host's hooks are untouched. `openclaw` is exempt — it mounts `/workspace` from a Podman named volume, not a host bind-mount, so there is no host `.git` tree for a caged agent to reach or pivot to. Closes #170.
- **`claude-code` workspace bind-mount no longer exposes the project-local `.claude/settings.json` to cage writes (cage→cage hooks-injection).** The same `${PROJECT_DIR}:/workspace:rw` bind-mount exposed the project-level `.claude/` to cage writes. Claude Code reads project-level `.claude/settings.json` on launch and honors its `hooks` block (shell commands run with tool privileges), so cage A could write a malicious `hooks.PreToolUse` and the next cage B's Claude Code would execute it on launch — a cage→cage hooks-injection chain proven across three cage generations in the CTF. The claude-code scaffold's `tmpfs:` block now overlays `/workspace/.claude/` with an empty, `noexec` tmpfs. This is the **project-level** `.claude/` under `/workspace`, not the cage's own `~/.claude` home (which holds `CLAUDE.md`, login creds, and in-cage settings, all intact); project-level Claude config/commands/subagents must be baked into the cage image. codex and pi were investigated and do not read an analogous project-local executable-config file (no `hooks`-equivalent block), so they get the `.git/hooks` mask only. Note: these tmpfs masks are honored by the container (rootless-podman) and `vm` backends; the `apple-container` backend does not yet honor `tmpfs:` entries (parity tracked in #120), so the masks are decorative there until that work lands. Closes #173.
...
- **IMAP relays gained `policy.write_mode` and `policy.folder_denylist`.** The previous choice was binary — `readonly: true` blocks `MOVE`/`COPY`/`STORE` along with everything else, `readonly: false` permits `EXPUNGE` — which left no way to express the common case of letting an agent tidy a mailbox without ever destroying anything in it. `write_mode: organise` permits reads, `MOVE`, `COPY` and flagging, while refusing `EXPUNGE`, `UID EXPUNGE`, `CLOSE`, `APPEND`, `DELETE`, `RENAME`, the ACL/metadata setters, and any `STORE` that would **add** the `\Deleted` flag (removing it is allowed — that un-deletes). `CLOSE` is refused because RFC 3501 §6.4.2 makes it expunge `\Deleted` messages as a side effect, so blocking `EXPUNGE` alone would leave the destructive path open behind an innocuous verb; refusing the flag as well means an expunge reached by any other route has nothing to reap. `folder_denylist` names mailboxes that may never be selected and wins over `folder_allowlist`, matched case-insensitively because servers disagree on the case of special-use names and failing open is the wrong direction here. `CREATE` is permitted — the mode's test is "refuse what destroys, permit what is recoverable", and a new folder can be deleted again, whereas requiring the owner to hand-create every destination first defeats the purpose; `DELETE` and `RENAME` stay refused because one removes a folder and its contents and the other silently breaks server-side filters that name folders. Since `MOVE`/`COPY` destinations are not policed, `folder_denylist: [Trash]` with `write_mode: organise` yields an agent that can file mail into Trash but cannot open Trash to purge it. `readonly` keeps working and maps onto `write_mode` (`true` → `none`, `false`/absent → `full`); setting both to contradicting values is a config error rather than a precedence puzzle.
...
- **`--no-cache`/`--pull` now rebuild the egress image and the in-VM cage image, so the flags behave consistently across all three isolation backends.** Previously the `container` backend's `build_artifacts` accepted `--no-cache`/`--pull` but dropped them when building the shared `agentcage-egress` image, and the `vm` backend ignored them entirely (the egress build, the in-VM scaffold/cage build, and the direct image pull all ran cache-hot) — so `--no-cache`/`--pull` on those backends silently left stale egress/cage layers in place, contradicting their documented meaning. Both backends now forward the flags to every build/pull step inside their build path, matching what the `apple-container` backend already did. This affects `agentcage run`, `cage create`, and `cage update` alike.
- **`agentcage run` now fails fast on a missing required secret instead of starting a half-configured cage.** A `run` cage whose scaffold declared a `secret_injection` rule (e.g. claude-code's `ANTHROPIC_API_KEY`) that wasn't supplied via `--set-secret` or resolvable from a configured `source:` (`env:`/`cmd:`/`systemd-creds:`) previously had that rule silently stripped — the cage came up, the egress proxy forwarded the unswapped placeholder, and the agent failed to authenticate with no clear signal. `run` now runs the same pre-flight secret check as `cage create` (before saving any state, ahead of the volume-dir and port checks) and aborts with the list of missing keys and a ready-to-paste `agentcage run <scaffold> -s KEY=VALUE` hint. Every scaffold-declared secret is mandatory — there is no "optional secret" concept — so an agent that would otherwise authenticate without a key (e.g. claude-code via interactive OAuth `/login`) must be given one, or be launched from a persistent cage created with `agentcage init` whose config can be edited. On `vm`/`apple-container` hosts without host podman there's no secret store to check against, so the check is skipped, exactly as in `create`.
