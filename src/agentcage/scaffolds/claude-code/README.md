...
The scaffold also tmpfs-masks two executable-config surfaces under the workspace bind-mount as defense-in-depth against cage→host and cage→cage pivots:

- `/workspace/.git/hooks/` — a caged agent can't plant a git hook (`pre-commit`, `post-checkout`, …) that the next host-side `git commit` runs as the host user (#170). The cage sees an empty, transient `.git/hooks/` (writes vanish on stop); the host's hooks are untouched. `notmpcopyup` is what makes it empty: the mask is `noexec`, so a copy of the host's hooks could never run in the cage anyway (#328).
- `/workspace/.claude/` — the project-local `.claude/` (read from the RW workspace mount) is masked so a caged agent can't plant a malicious `.claude/settings.json` `hooks` block that Claude Code in another cage honors on launch (#173). This is the **project-level** `.claude/` under `/workspace`, not the cage's own `~/.claude` home (which holds `CLAUDE.md`, login creds, and in-cage settings, all intact). This mask carries `tmpcopyup`, so the cage still *reads* the project's settings, commands and subagents (#328) — they do not have to be baked into the cage image. What the cage gets is a copy, owned by the cage user so the agent can edit it; every write dies with the cage and none of it reaches the host.
