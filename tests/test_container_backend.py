...
class TestStartGrantsWatcher:
    """Fix 1 (reboot durability): ``ContainerBackend.start`` starts the
    grants watcher when its unit file exists, but it never ENABLED the
    unit — so after a host reboot the watcher stayed dead unless the
    operator had once run ``domain add --expires-in``. start now also
    calls ``systemd.enable_unit`` (best-effort, mirroring the start
    call's tolerance)."""

    @staticmethod
    def _backend_with_grants_unit(tmp_path):
        """ContainerBackend whose grants unit file exists under tmp_path."""
        backend = ContainerBackend()
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        # grants_unit_path() = user_unit_dir() / f"{name}-grants.service";
        # user_unit_dir() is independent of unit_dir(), so stub it too.
        (unit_dir / "myapp-grants.service").write_text("[Unit]\n")
        return backend, unit_dir

    def test_enables_grants_unit_when_present(self, tmp_path):
        backend, unit_dir = self._backend_with_grants_unit(tmp_path)
        with patch("agentcage.backends.container.systemd") as mock_sd, \
             patch.object(backend, "unit_dir", return_value=Path("/fake")), \
             patch.object(backend, "user_unit_dir", return_value=unit_dir):
            backend.start("myapp")

        mock_sd.start_unit.assert_any_call("myapp-grants.service")
        mock_sd.enable_unit.assert_called_once_with("myapp-grants.service")

    def test_skips_enable_when_unit_absent(self, tmp_path):
        """No grants unit file → no start, no enable (idempotent / safe)."""
        backend = ContainerBackend()
        with patch("agentcage.backends.container.systemd") as mock_sd, \
             patch.object(backend, "unit_dir", return_value=Path("/fake")), \
             patch.object(backend, "user_unit_dir",
                          return_value=tmp_path / "empty"):
            (tmp_path / "empty").mkdir()
            backend.start("myapp")

        mock_sd.enable_unit.assert_not_called()
        # start_unit only for the cage service, never the grants watcher.
        mock_sd.start_unit.assert_called_once_with("myapp-cage.service")

    def test_enable_failure_does_not_break_start(self, tmp_path, capsys):
        """enable is best-effort: a failure warns but the cage still
        started (mirrors the start call's tolerance)."""
        backend, unit_dir = self._backend_with_grants_unit(tmp_path)
        with patch("agentcage.backends.container.systemd") as mock_sd, \
             patch.object(backend, "unit_dir", return_value=Path("/fake")), \
             patch.object(backend, "user_unit_dir", return_value=unit_dir):
            mock_sd.enable_unit.side_effect = RuntimeError("dbus down")
            # Must not raise.
            backend.start("myapp")

        err = capsys.readouterr().err
        assert "failed to enable grants watcher" in err

    def test_start_failure_skips_enable(self, tmp_path, capsys):
        """If the watcher's start itself fails, we still attempt enable
        (the unit file is present; an enabled-but-dead unit will be
        retried by systemd on next boot). This mirrors the cli helper's
        own best-effort pairing."""
        backend, unit_dir = self._backend_with_grants_unit(tmp_path)
        with patch("agentcage.backends.container.systemd") as mock_sd, \
             patch.object(backend, "unit_dir", return_value=Path("/fake")), \
             patch.object(backend, "user_unit_dir", return_value=unit_dir):
            mock_sd.start_unit.side_effect = [
                None,  # myapp-cage.service start
                RuntimeError("unit not loaded"),  # grants start fails
            ]
            backend.start("myapp")

        mock_sd.enable_unit.assert_called_once_with("myapp-grants.service")
        err = capsys.readouterr().err
        assert "failed to start grants watcher" in err
