"""The traffic watcher — the in-egress, after-the-fact LLM auditor (egress side).

``data/proxy/watcher.py``: the digest builder's secret hygiene, the capture
tail (torn-line / rotation safe, staged-commit), the fail-closed review, the
drain-and-retry scan semantics, and the narrowing-only revocation path; plus
the addon's audit-ring funnel and the watcher's hot-reload lifecycle.

The proxy modules are imported the same way the other addon tests do (proxy dir
...
Rust; it moved to ``tests/test_watcher_host.py``. Test names are unchanged on
both sides so failures stay greppable against history.
...
class TestBuildDigest:
    ...
    def test_control_records_are_not_counted_as_flows(self):
        """policy_* records ride in policy_events, not the flow totals.

        They carry no decision/method/host, so counting them alongside
        traffic inflated ``flows`` and produced an empty-string bucket in
        ``decisions``/``methods``. On jacque the watcher spent a finding
        per interval reporting that bucket as a "malformed empty
        policy_introspect record" — noise generated by the digest itself.
        """
        entries = [
            {"ts": "t1", "decision": "allowed", "host": "a.example",
             "method": "GET"},
            {"kind": "policy_introspect", "ts": "t2", "path": "/v1/allowlist"},
            {"kind": "policy_request", "ts": "t3", "domain": "x.example",
             "decision": "denied"},
        ]
        d = build_digest(entries, [], [e for e in entries if e.get("kind")],
                         granted=[], baseline=[], max_flows=10)
        assert d["totals"]["flows"] == 1
        assert d["totals"]["decisions"] == {"allowed": 1}
        assert d["totals"]["methods"] == {"GET": 1}
        assert "" not in d["totals"]["decisions"]
        assert "" not in d["totals"]["methods"]
        # ...but the decider's own record still reaches the model.
        assert len(d["policy_events"]) == 2

    def test_relay_records_still_count_as_traffic(self):
        """A relay record has a ``kind`` too — and IS traffic to audit.

        The control-plane filter keys on the policy_/watcher_ prefixes
        precisely so mail-relay activity keeps reaching the digest.
        """
        entries = [
            {"kind": "imap_command", "ts": "t1", "relay": "migadu-imap",
             "command": "SELECT", "decision": "allowed"},
            {"kind": "smtp_command", "ts": "t2", "relay": "migadu-smtp",
             "decision": "blocked", "host": "smtp.example"},
        ]
        d = build_digest(entries, [], [], granted=[], baseline=[],
                         max_flows=10)
        assert d["totals"]["flows"] == 2
        assert d["totals"]["decisions"] == {"allowed": 1, "blocked": 1}
        # No method on a relay record — and no "" bucket invented for it.
        assert d["totals"]["methods"] == {}
...
class TestCaptureTail:
    ...
    # sane budget). Past the derived line cap it is dropped instead.
    def test_oversized_line_advances_offset_instead_of_stalling(
        ...
        w._capture_path = str(cap)
        w._window = 3600
        w._line_cap = 500  # derived per cage from capture.max_body_size
        now = datetime.now(timezone.utc)
        samples, off = self._commit(w, now)
        assert samples == []
    ...
    # bigger than the line cap ends its read mid-line, so every chunk
    ...
    def test_large_backlog_loses_no_entries(self, tmp_path, monkeypatch):
        ...
        w._capture_path = str(cap)
        w._window = 3600
        w._line_cap = 1200  # derived per cage from capture.max_body_size
