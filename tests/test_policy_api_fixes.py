...
class TestWatcherPlistHasAUsablePath:
    """The launchd watcher must be able to find ``limactl``.

    launchd starts agents with a bare ``/usr/bin:/bin:/usr/sbin:/sbin`` and
    does not inherit the operator's shell PATH. A vm cage's watcher shells
    out to ``limactl`` every tick to pull the guest-local grants overlay;
    Homebrew puts it in ``/opt/homebrew/bin``. Without an explicit PATH the
    watcher loaded, stayed up, and failed every tick with
    ``FileNotFoundError`` — a live watcher that could never promote a grant.
    """

    def test_path_includes_homebrew_and_the_binary_dir(self):
        from agentcage.watcher import _watcher_path

        path = _watcher_path("/opt/pipx/venvs/agentcage/bin/agentcage")
        entries = path.split(":")
        assert "/opt/pipx/venvs/agentcage/bin" in entries, (
            "the agentcage binary's own directory must be on PATH"
        )
        assert "/opt/homebrew/bin" in entries, (
            "Homebrew's bin must be on PATH so limactl resolves"
        )
        assert "/usr/local/bin" in entries, "Intel Homebrew prefix"
        for base in ("/usr/bin", "/bin", "/usr/sbin", "/sbin"):
            assert base in entries

    def test_plist_declares_the_path(self, tmp_path):
        from agentcage.watcher import install_grants_watcher_plist

        plist = tmp_path / "io.agentcage.x.grants.plist"
        install_grants_watcher_plist(
            "x", log_dir=tmp_path / "logs", interval=5, plist_path=plist,
        )
        xml = plist.read_text()
        assert "<key>EnvironmentVariables</key>" in xml
        assert "<key>PATH</key>" in xml
        assert "--interval" in xml and "<string>5</string>" in xml
