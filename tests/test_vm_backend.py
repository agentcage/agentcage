...
# ---------------------------------------------------------------------------
# Grants-watcher lifecycle (install on deploy, uninstall on destroy/stop)
# ---------------------------------------------------------------------------

class TestVmGrantsWatcherInstall:
    """Fix 1 (HIGH): a VM cage created with ``domains.auto.enable: true``
    used to never install the host-side grants watcher — LLM-decided
    grants in the guest-local overlay were never promoted into the
    operator's cage.yaml. ``_deploy_cage`` now calls
    ``cli._ensure_grants_watcher(name, config)`` at the end of a
    successful deploy (late import to dodge the backend→cli cycle)."""

    @staticmethod
    def _active_inst() -> MagicMock:
        """Mock LimaInstance whose guest reports every service active."""
        inst = MagicMock()
        inst.name = "agentcage-testcage"
        # Every exec (is-active, daemon-reload, start, echo ~, mkdir, ...)
        # returns an "active"/empty MagicMock — the deploy only cares that
        # is-active reports "active" so it skips the (mocked) start path.
        inst.exec.return_value = MagicMock(
            returncode=0, stdout="active\n", stderr="")
        return inst

    def test_deploy_cage_calls_ensure_grants_watcher(self):
        backend = VmBackend()
        inst = self._active_inst()
        config = _make_config()

        with patch.object(backend, "_instance", return_value=inst), \
             patch.object(backend, "build_artifacts"), \
             patch.object(backend, "_bridge_secrets"), \
             patch.object(backend, "_create_pending_secrets"), \
             patch("agentcage.backends.vm.push_config_files"), \
             patch("agentcage.backends.vm.ensure_grants_dir"), \
             patch("pathlib.Path.exists", return_value=True), \
             patch("pathlib.Path.iterdir", return_value=iter([])), \
             patch("agentcage.cli._ensure_grants_watcher") as mock_ensure:
            backend._deploy_cage("testcage", inst, config)

        mock_ensure.assert_called_once_with("testcage", config)

    def test_deploy_cage_skips_watcher_when_config_none(self):
        """No live Config (state load failed) → don't install, because the
        helper would fall through to the container-backend path and write
        the wrong unit for a vm cage. Idempotent helper handles None
        gracefully, but the backend guards to avoid the mis-route."""
        backend = VmBackend()
        inst = self._active_inst()
        ...
        with patch.object(backend, "_instance", return_value=inst), \
             ...
             patch("pathlib.Path.iterdir", return_value=iter([])), \
             patch("agentcage.cli._ensure_grants_watcher") as mock_ensure:
            backend._deploy_cage("testcage", inst, config=None)

        mock_ensure.assert_not_called()

    def test_deploy_cage_does_not_fail_if_watcher_install_raises(self, capsys):
        """The helper is best-effort; a failure must not break the deploy."""
        backend = VmBackend()
        inst = self._active_inst()
        config = _make_config()
        ...
        with patch.object(backend, "_instance", return_value=inst), \
             ...
             patch("pathlib.Path.exists", return_value=True), \
             patch("pathlib.Path.iterdir", return_value=iter([])), \
             patch("agentcage.cli._ensure_grants_watcher",
                   side_effect=RuntimeError("boom")):
            # Must not raise.
            backend._deploy_cage("testcage", inst, config)

        err = capsys.readouterr().err
        assert "could not install grants watcher" in err


class TestVmGrantsWatcherUninstall:
    """Fix 3 (MEDIUM): ``cage destroy`` on a VM cage used to leak the
    host-side watcher — an ENABLED systemd unit polling a nonexistent
    cage (Linux) or a ``KeepAlive=true`` launchd agent relaunching
    forever (macOS). ``destroy_resources`` now calls
    ``uninstall_grants_watcher`` (best-effort)."""

    def test_destroy_calls_uninstall_with_platform_darwin_flag(self):
        backend = VmBackend()
        mock_inst = MagicMock()
        mock_inst.exists.return_value = False

        with patch.object(backend, "_instance", return_value=mock_inst), \
             patch.object(backend, "unit_dir",
                          return_value=Path("/nonexistent/path")), \
             patch("agentcage.backends.vm.sys.platform", "darwin"), \
             patch("agentcage.watcher.uninstall_grants_watcher") as mock_un:
            backend.destroy_resources("testcage")

        mock_un.assert_called_once_with("testcage", darwin=True)

    def test_destroy_calls_uninstall_with_linux_flag(self):
        backend = VmBackend()
        mock_inst = MagicMock()
        mock_inst.exists.return_value = False
        ...
        with patch.object(backend, "_instance", return_value=mock_inst), \
             patch.object(backend, "unit_dir",
                          return_value=Path("/nonexistent/path")), \
             patch("agentcage.backends.vm.sys.platform", "linux"), \
             patch("agentcage.watcher.uninstall_grants_watcher") as mock_un:
            backend.destroy_resources("testcage")

        mock_un.assert_called_once_with("testcage", darwin=False)

    def test_destroy_does_not_fail_if_uninstall_raises(self):
        backend = VmBackend()
        mock_inst = MagicMock()
        mock_inst.exists.return_value = True
        mock_inst.name = "agentcage-testcage"
        ...
        with patch.object(backend, "_instance", return_value=mock_inst), \
             patch.object(backend, "unit_dir",
                          return_value=Path("/nonexistent/path")), \
             patch("agentcage.watcher.uninstall_grants_watcher",
                   side_effect=RuntimeError("boom")):
            # Must not raise.
            removed = backend.destroy_resources("testcage")
...
class TestVmGrantsWatcherStop:
    """``VmBackend.stop`` mirrors ``ContainerBackend.stop``: it stops the
    host-side grants watcher IF its supervisor file exists (so a stopped
    cage doesn't leave a watcher spinning against a dead VM), but leaves
    the file in place for a later ``start`` to re-activate."""

    def test_stop_linux_stops_watcher_when_unit_exists(self):
        backend = VmBackend()
        mock_inst = MagicMock()
        mock_inst.is_running.return_value = True
        ...
        with patch.object(backend, "_instance", return_value=mock_inst), \
             patch("agentcage.backends.vm.sys.platform", "linux"), \
             patch("agentcage.watcher.grants_watcher_unit_path",
                   return_value=Path("/existing/unit.service")) as mock_path, \
             patch("pathlib.Path.exists", return_value=True), \
             patch("agentcage.systemd.stop_unit") as mock_stop:
            backend.stop("testcage")

        mock_stop.assert_called_once_with("testcage-grants.service")
        mock_path.assert_called_once_with("testcage")

    def test_stop_linux_skips_watcher_when_unit_absent(self):
        backend = VmBackend()
        mock_inst = MagicMock()
        mock_inst.is_running.return_value = True
        ...
        with patch.object(backend, "_instance", return_value=mock_inst), \
             patch("agentcage.backends.vm.sys.platform", "linux"), \
             patch("agentcage.watcher.grants_watcher_unit_path",
                   return_value=Path("/missing/unit.service")), \
    ...
    def test_stop_darwin_boots_out_when_plist_exists(self):
        backend = VmBackend()
        mock_inst = MagicMock()
        mock_inst.is_running.return_value = True
        ...
        with patch.object(backend, "_instance", return_value=mock_inst), \
             patch("agentcage.backends.vm.sys.platform", "darwin"), \
             patch("agentcage.watcher.grants_watcher_plist_path",
                   return_value=Path("/existing/plist")) as mock_plist, \
             patch("pathlib.Path.exists", return_value=True), \
             patch("subprocess.run") as mock_run:
            backend.stop("testcage")

        mock_plist.assert_called_once_with("testcage")
        # launchctl bootout for gui/<uid>/io.agentcage.testcage.grants
        bootout_calls = [
            c for c in mock_run.call_args_list
            if c.args and "launchctl" in str(c.args[0])
        ]
        assert len(bootout_calls) == 1
        cmd = bootout_calls[0].args[0]
        assert "bootout" in cmd
        assert "io.agentcage.testcage.grants" in " ".join(cmd)


class TestUninstallGrantsWatcher:
    """Direct coverage of ``agentcage.watcher.uninstall_grants_watcher``
    for both platforms (the destroy path delegates to it)."""

    def test_darwin_boots_out_and_unlinks_plist(self, tmp_path):
        from agentcage.watcher import uninstall_grants_watcher

        plist = tmp_path / "io.agentcage.testcage.grants.plist"
        plist.write_text("<plist/>")

        with patch("agentcage.watcher.grants_watcher_plist_path",
                   return_value=plist), \
             patch("agentcage.watcher.subprocess.run") as mock_run, \
             patch("agentcage.watcher.os.getuid", return_value=501):
            uninstall_grants_watcher("testcage", darwin=True)

        # bootout attempted against gui/501/io.agentcage.testcage.grants
        bootout_calls = [
            c for c in mock_run.call_args_list
            if c.args and "bootout" in str(c.args[0])
        ]
        assert len(bootout_calls) == 1
        cmd = bootout_calls[0].args[0]
        assert "gui/501/io.agentcage.testcage.grants" in cmd
        # Plist file unlinked.
        assert not plist.exists()

    def test_darwin_idempotent_when_plist_missing(self, tmp_path):
        from agentcage.watcher import uninstall_grants_watcher

        plist = tmp_path / "missing.plist"  # never written

        with patch("agentcage.watcher.grants_watcher_plist_path",
                   return_value=plist), \
             patch("agentcage.watcher.subprocess.run"):
            # Must not raise even though the plist is absent.
            uninstall_grants_watcher("testcage", darwin=True)

        assert not plist.exists()

    def test_linux_stops_disables_unlinks_unit(self, tmp_path):
        from agentcage.watcher import uninstall_grants_watcher
        import agentcage.watcher as wmod

        unit = tmp_path / "testcage-grants.service"
        unit.write_text("[Unit]\n")

        fake_sd = MagicMock()
        with patch("agentcage.systemd", fake_sd), \
             patch.object(wmod, "grants_watcher_unit_path",
                          return_value=unit):
            uninstall_grants_watcher("testcage", darwin=False)

        fake_sd.stop_unit.assert_called_once_with("testcage-grants.service")
        fake_sd.disable_unit.assert_called_once_with("testcage-grants.service")
        fake_sd.daemon_reload.assert_called_once()
        assert not unit.exists()

    def test_linux_idempotent_when_unit_missing_and_calls_raise(self, tmp_path):
        """stop/disable/daemon_reload may raise (unit never existed); the
        teardown still unlinks whatever file is present and never raises."""
        from agentcage.watcher import uninstall_grants_watcher
        import agentcage.watcher as wmod
        ...
        fake_sd = MagicMock()
        fake_sd.stop_unit.side_effect = RuntimeError("no such unit")
        fake_sd.disable_unit.side_effect = RuntimeError("not loaded")
        fake_sd.daemon_reload.side_effect = RuntimeError("dbus gone")

        with patch("agentcage.systemd", fake_sd), \
             patch.object(wmod, "grants_watcher_unit_path",
                          return_value=unit):
            # Must not raise despite every systemd call failing.
            uninstall_grants_watcher("testcage", darwin=False)
...
class TestGrantsWatcherUnitPath:
    """``watcher.grants_watcher_unit_path`` mirrors the per-backend
    ``grants_unit_path`` helpers so the uninstall path doesn't need to
    import a backend module."""

    def test_returns_systemd_user_dir(self):
        from agentcage.watcher import grants_watcher_unit_path

        expected = Path(
            os.path.expanduser("~/.config/systemd/user")
        ) / "mycage-grants.service"
        assert grants_watcher_unit_path("mycage") == expected
