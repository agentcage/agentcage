...
**apple-container — tmpfs options are not applied.** `container.tmpfs` mounts are created, including the scaffold masks over `/workspace/.git/hooks/` and `/workspace/.claude/`, but Apple's runtime takes a bare path so `noexec`, `nosuid`, `nodev` and `size=` are dropped. Set `container.memory` to bound an unlimited tmpfs. `tmpcopyup`/`notmpcopyup` are the exception: agentcage emulates copy-up by seeding the tmpfs from a read-only mount of the covered host directory at cage-init time. See [tmpfs mounts](../reference/configuration.md#tmpfs-mounts).
