...
3. **DNS filtering** -- When using allowlist mode, the dnsmasq sidecar returns NXDOMAIN for non-allowlisted domains, blocking DNS-level exfiltration. DNS query logging is enabled by default for forensic analysis.
